...
class ChatInterface(tk.Frame):
    ...
    def on_send_button_clicked(self):
        ...
        recipient = self.users_listbox.get(...)

        content = self.message_input.get()

        # NOUVEAU: Envoyer avec signature

        self.client.send_private_message(recipient, content)

        # Afficher localement

        self.display_message(f"Moi: {content}")

        self.message_input.delete(0, tk.END)

    def display_received_message(self, sender, content, signature_valid):

        indicator = "🔒" if signature_valid else "🔓"

        text = f"{indicator} {sender}: {content}"
